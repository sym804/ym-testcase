...
**계정 열거 방지.** 계정 복구 요청 접수 응답이 대상 존재 여부와 무관하게 동일하다. 검증 실패도
단계를 구분하지 않고 같은 401 이다. 이는 계정 복구 엔드포인트에 한정된 보장이며 시스템 전체의
속성은 아니다. 회원가입 흐름의 `GET /api/auth/check-username` 은 인증 없이 아이디 존재 여부를
그대로 답하고, `POST /api/auth/register` 도 이미 등록된 아이디면 400 으로 이를 알려준다. 둘 다
가입 흐름에서 의도한 동작이라 여기서는 바꾸지 않는다.
