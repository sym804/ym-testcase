...
## v1.6.10.5 (2026-09-23) - [fix] 우선순위 글자색 대비, 리포트 빈 값 표기 통일
...
| Frontend | 1.6.7.1 | **1.6.7.2** | patch +1 |
| Backend | 1.6.5.0 | **1.6.5.1** | patch +1 |

### 이슈

- SYM-129 우선순위 글자색이 접근성 대비 기준(4.5:1)에 못 미친다 (bug/minor/frontend)
- SYM-130 리포트에서 분류와 우선순위가 비어 있는 TC 의 표기가 곳마다 다르다 (bug/trivial/frontend)

### 변경

- 우선순위 색을 --priority-* 테마 토큰으로. 그리드 hover·선택 상태까지 라이트/다크 모두 4.5:1 이상
- 리포트 빈 분류·우선순위: 화면 "(미분류)"/"(미지정)", PDF/엑셀 "(none)", JSON null. 목록 우선순위도 번역

### 영향

- 우선순위 색이 라이트는 조금 진하게, 다크는 조금 밝게 바뀐다. 리포트 API 의 분류 "Uncategorized" 가 null 이 된다
...
## v1.6.10.3 (2026-09-23) - [docs] 문서와 매뉴얼 최신화

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.10.2 | **1.6.10.3** | patch +1 |
| Frontend | 1.6.7.0 | **1.6.7.1** | patch +1 |
...
- 앱 매뉴얼: 시트가 여럿이면 "전체" 로 열리는 점, 우선순위 색(TC 관리, 테스트 수행)
- README, CONTRIBUTING, frontend/README, rules/release_process: 리포트와 이슈 관리 도구 기능, 테스트 수, 타입 검사와 pytest 명령
- Confluence 사용자 매뉴얼과 초안: 리포트 절, 시트 기본 선택, 우선순위 색
...
## v1.6.10.2 (2026-09-23) - [chore] 우선순위 옛 값 '중간' 정리

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.10.1 | **1.6.10.2** | patch +1 |

### 이슈

- SYM-128 우선순위 옛 값 '중간' 을 '보통' 으로 일괄 변경한다 (enhancement/major/db)

### 변경

- ymseo_test 프로젝트 TC 98건의 우선순위를 '중간' 에서 '보통' 으로 일괄 변경(일괄 변경 API, 변경 이력 98건 기록)

### 영향

- 해당 TC 가 TC 관리와 테스트 수행 그리드에서 '보통' 색으로 보인다. 코드 변경 없음

---

## v1.6.10.1 (2026-09-23) - [fix] E2E 행 선택 테스트 간헐 실패

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.10.0 | **1.6.10.1** | patch +1 |

### 이슈

- SYM-127 E2E TC-TCM-007/008 이 새 행 자동 선택과 겹쳐 간헐 실패한다 (bug/minor/etc)

### 변경

- 두 테스트가 체크박스를 누르지 않고, 행 추가 뒤 100ms 에 걸리는 자동 선택이 끝나기를 기다린다
...
## v1.6.10.0 (2026-09-23) - [fix] 테스트 수행 시트 이슈 링크 열기

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.9.0 | **1.6.10.0** | fix +1 |
| Frontend | 1.6.6.0 | **1.6.7.0** | fix +1 |

### 이슈

- SYM-123 테스트 수행 그리드의 이슈 키가 Jira 링크로 이어지지 않는다 (bug/minor/frontend)

### 변경

- Issue Link 칸에 이슈 키나 주소가 있으면 옆에 ↗ 를 붙여 이슈 관리 도구의 이슈를 새 탭으로 연다. 주소 규칙은 리포트와 같다
- 글자는 링크로 만들지 않는다. 더블클릭 편집은 그대로다. ↗ 의 더블클릭은 편집으로 번지지 않게 막고, ↗ 를 누른 뒤 포커스는 셀로 돌아간다
- 매뉴얼과 docs 문구를 구현에 맞춤

### 영향

- 이슈 키는 설정 탭에 이슈 관리 도구 주소가 있을 때만 링크된다. 주소 값은 항상 링크된다

---

## v1.6.9.0 (2026-09-23) - [fix] TC 관리 시트 선택 불일치, 수행 시트 우선순위 색, 이슈 관리 도구 주소

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.8.0 | **1.6.9.0** | fix +1 |
| Frontend | 1.6.5.0 | **1.6.6.0** | fix +1 |

### 이슈

- SYM-124 시트가 여럿인 프로젝트에서 TC 관리를 열면 사이드바와 표의 시트가 다르다 (bug/major/frontend)
- SYM-125 테스트 수행 시트의 우선순위에 TC 시트와 같은 색을 넣는다 (enhancement/minor/frontend)
- SYM-126 이슈 링크 설정을 Jira 전용이 아닌 이슈 관리 도구 주소로 바꾼다 (enhancement/minor/frontend)

### 변경

- TC 관리: 시트가 여럿이면 "전체" 로 열고, 하나면 그 시트를 선택. 조회와 고급 필터가 요청 순번을 공유해 늦게 온 응답을 버린다
- 우선순위 값과 색을 `utils/priority.ts` 로 공용화. 수행 그리드 칸과 필터 메뉴에 적용
- 우선순위 4단계(매우 높음/높음/보통/낮음). "매우 낮음" 제거
- "Jira Base URL" 을 "이슈 관리 도구 주소" 로 변경. Linear 주소와 `{key}` 자리표시자 지원. 설정 탭에 입력칸 추가

### 영향

- 시트가 여럿인 프로젝트는 처음에 행 추가가 꺼져 있다. 시트를 고르면 켜진다
- 목록 밖의 우선순위 값(중간, High 등)은 색 없이 표시된다
- API 필드 이름 `jira_base_url` 은 그대로다
...
## v1.6.7.1 (2026-09-23) - [docs] 릴리즈 태그 소급과 누락 검사

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.7.0 | **1.6.7.1** | patch +1 |

### 이슈

- SYM-117 릴리즈 태그가 45개 빠져 있었다 (bug/minor/etc)

### 변경

- 커밋 메시지의 버전 표기로 릴리즈 커밋을 찾아 태그 34개를 소급해 달고 푸시했다. v1.2.2.0 과 v0.x 초기 11개는 커밋을 특정하지 못해 남겼다
- `test_version_consistency.py` 가 지난 릴리즈의 태그 존재를 확인한다. 최신 절 하나는 면제하므로 한 박자 늦게 잡는다
