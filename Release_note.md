...
```
YM TestCase System  v1.4.0.0  (2026-09-08)
├── Frontend       v1.4.0.0
├── Backend        v1.4.0.0
└── Database       v0.8.0.0
```
...
- 새 테스트 수행 모달에 "포함할 시트" 추가. 기본은 전체 선택이고 체크를 풀어 줄인다. 시트가 하나뿐이면 나오지 않는다
- 고른 범위를 `test_runs.sheet_names` 에 저장한다. 생성 시점의 필터가 아니라 수행의 범위라서, 진행 중 수행이 새 TC 를 흡수할 때도 같은 조건을 건다
- 전부 고르면 범위를 저장하지 않는다. 범위 없음은 프로젝트 전체이고 기존 수행과 같다
- 복제하면 범위도 따라간다. 폴더와 없는 시트는 범위로 받지 않고, 범위 밖 TC 의 결과 제출은 400 으로 거부한다
- 시트 이름을 바꾸거나 지우면 그 시트를 담은 수행의 범위도 함께 고친다
- 대시보드 총 건수를 수행이 담은 TC 수로 바꿨다. summary, priority, category, rounds, 전역 개요. 분자도 같은 기준으로 좁힌다
- 결과 저장이 거부되면 서버가 준 사유를 띄우고 화면을 서버 값으로 되돌린다
- 수행 상세의 시트 탭이 그 수행의 범위만 보여 준다

### 영향

- 기존 수행은 `sheet_names` 가 NULL 이라 동작이 달라지지 않는다
- 일부 시트만 담은 수행의 대시보드 수치가 달라진다. 결제 시트 4건을 전부 PASS 한 수행이 36.4% 에서 100% 로 정정된다
- 전역 개요의 총계가 프로젝트 전체 TC 수가 아니라 최신 수행이 담은 수로 바뀐다
...
- 첨부 맵을 통째로 갈아끼우지 않고 병합한다. 조회가 도는 동안 올리거나 지운 첨부가 응답 한 번에 되돌아가지 않는다
- 런 상세 요청에 세대 번호를 붙여 늦게 도착한 이전 런의 응답을 버린다
- 시트 탭만 바꿀 때는 첨부를 다시 받지 않는다
- `attachments.test_result_id` 인덱스 추가. 조회가 첨부 테이블 전체 스캔을 타고 있었다(실측 query plan `SCAN a`)
- 테스트 11건 추가. 공용 ag-grid 모킹이 rowData 길이만 출력해 셀 렌더러가 한 번도 실행되지 않았고, 그래서 이 결함이 통과 415건 안에서 그대로 지나갔다
