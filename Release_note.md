...
```
YM TestCase System  v1.6.7.0  (2026-09-22)
├── Frontend       v1.6.4.0
├── Backend        v1.6.4.0
└── Database       v0.9.2.0
```
...
## v1.6.7.0 (2026-09-22) - [fix] 그리드 셀 텍스트 복사, 더블클릭 편집

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.6.0 | **1.6.7.0** | fix +1 |
| Frontend | 1.6.3.0 | **1.6.4.0** | fix +1 |

### 이슈

- SYM-114 테스트 수행 시트에서 셀 텍스트를 끌어서 복사할 수 없다 (enhancement/minor/frontend)
- SYM-115 TC 그리드가 클릭 한 번에 편집 모드로 들어간다 (bug/minor/frontend)
- SYM-116 매뉴얼이 구현되지 않은 Ctrl+C/V 셀 복사 기능을 안내한다 (bug/trivial/etc)

### 변경

- 두 그리드에서 `singleClickEdit` 을 뺐다. 클릭 한 번은 셀 선택이고, 편집은 더블클릭·F2·바로 타이핑으로 시작한다. 매뉴얼은 이전부터 더블클릭이라고 안내하고 있었고 구현만 달랐다
- `enableCellTextSelection` 과 `ensureDomOrder` 를 켰다. 셀 안의 글자를 끌어서 선택해 복사할 수 있다. 절차·기대 결과처럼 읽기 전용인 열에서도 된다. 두 값은 한 쌍이라 `ensureDomOrder` 가 빠지면 여러 행에 걸친 선택 범위가 어긋난다
- 매뉴얼 단축키 표의 `Ctrl+C/V` 항목을 "끌어서 선택한 글자 복사 / 편집 중 붙여넣기" 로 고쳤다. AG Grid 의 셀 범위 복사는 Enterprise 전용인데 Community 만 쓰는 이 제품의 표에 실려 있었다

### 영향

- 수행 시트에서 편집 가능한 열은 그대로 셋이다(실제 결과, 이슈 링크, 비고). TC 원문은 계속 읽기 전용이다
- 결과 드롭다운, Shift+클릭 범위 채우기, Ctrl+D, 행 드래그는 그대로 동작한다
- `ensureDomOrder` 가 DOM 순서를 화면 순서에 맞추므로 행 이동 애니메이션이 덜 부드러울 수 있다. 519건 시트에서 스크롤 지연은 관측되지 않았다
- 저장되는 데이터는 없다. 조작 방식만 바뀐다

---

## v1.6.6.0 (2026-09-21) - [feat] 테스트 수행 화면 우선순위 멀티 선택

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.5.0 | **1.6.6.0** | fix +1 |
| Frontend | 1.6.2.1 | **1.6.3.0** | fix +1 |

### 이슈

- SYM-113 테스트 수행 화면에서 우선순위를 하나만 고를 수 있다 (enhancement/minor/frontend)

### 변경

- Priority 필터를 단일 select 에서 체크박스 드롭다운으로 바꿨다. 고른 값 중 하나라도 맞으면 남고, Result/Category/검색어와는 AND 로 합쳐진다
- 버튼 라벨은 안 고르면 "전체", 하나면 그 값, 둘 이상이면 개수다. 값을 늘어놓으면 필터바 폭이 값 길이에 따라 흔들린다
- 우선순위를 비워 둔 TC 가 있으면 목록 맨 아래에 (미지정) 이 생긴다. 없으면 나오지 않는다
- 고른 값이 지금 결과 집합에 없어도 목록에 남긴다. 시트를 옮기면 그 값이 목록에서 빠져 체크를 풀 수단이 없어지고, 그리드가 빈 채로 잠겼다
- 바깥 클릭과 Esc 로 닫는다. 런이나 시트를 옮기면 열린 드롭다운을 닫는다
- 활성 버튼을 accent 배경에 흰 글자로 칠한다. accent 를 글자색으로 쓰면 다크에서 3.27:1 로 AA(4.5:1) 에 못 미쳤다. 이 변수는 흰 글자를 얹는 배경 기준으로 보정된 값이다(4.62:1, SYM-55 와 같은 패턴)

### 영향

- 값 하나만 체크하면 이전 단일 선택과 같게 동작한다
- 저장되는 데이터는 없다. 화면 상태만 바뀐다
- 훅 반환값 `filterPriority`(string) 가 `filterPriorities`(string[]) 와 `togglePriority` 로 바뀌었다. 이 훅은 `TestRunManager` 만 쓴다

---

## v1.6.5.0 (2026-09-18) - [fix] 시트가 여럿인 파일을 다시 임포트하면 행이 불어나던 문제

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.4.0 | **1.6.5.0** | fix +1 |
| Backend | 1.6.3.0 | **1.6.4.0** | fix +1 |

### 이슈

- SYM-112 시트가 여럿인 파일을 다시 임포트하면 두 번째 시트부터 실패한다 (bug/major/backend)

### 변경

- 재임포트 조회가 실패하면 채번이 접미사를 붙여 저장한 행을 이 시트에서 되짚는다. 접미사를 세어 올라가지 않고 맵에 있는 것 중 가장 작은 것을 골라, 세 번째 이후 시트와 50자로 잘린 ID 도 찾는다
- 되짚기는 `base` 가 프로젝트에 실제로 있을 때만 한다. 없으면 그 `base-2` 는 채번이 만든 것이 아니라 사람이 손으로 붙인 ID 라, 덮으면 데이터가 사라진다
- 파일이 직접 적은 ID 는 그 행이 스스로 이으므로 되짚기 대상에서 뺀다

### 영향

- 없음. 첫 임포트 결과는 예전과 같고 재임포트가 행을 늘리지 않는다

---

## v1.6.4.0 (2026-09-18) - [fix] 재임포트가 앞 행을 덮어 뒤 행 내용이 사라지던 문제

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.3.1 | **1.6.4.0** | fix +1 |
| Backend | 1.6.2.0 | **1.6.3.0** | fix +1 |

### 이슈

- SYM-51 TC ID 가 없는 파일을 다시 임포트하면 같은 행을 두 번 갱신할 수 있다 (bug/major/backend)

### 변경

- 한 파일 안에서 TC ID 가 겹치면 파서가 `-2`, `-3` 을 직접 붙인다. 채번은 저장 시점에 푸는 것이라 다음 임포트에서 같은 값을 재계산해 내지 못했다
- 파일에 적힌 ID 와 No 로 만든 ID 를 한 이름공간에서 센다. 나누면 둘이 부딪칠 때 같은 증상이 남는다
- 만들어지는 ID 자체는 예전과 같아 이미 임포트해 둔 데이터의 재임포트가 그대로 이어진다

### 영향

- 없음. 재임포트가 각 행을 제 자리에 갱신한다. 시트를 가로지르는 경우는 SYM-112 로 분리했다

---

## v1.6.3.1 (2026-09-18) - [fix] 다크 테마 명암비가 AA 에 못 미치던 토큰

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.3.0 | **1.6.3.1** | patch +1 |
| Frontend | 1.6.2.0 | **1.6.2.1** | patch +1 |

### 이슈

- SYM-55 다크 테마에서 선택 상태 버튼과 탭의 글자 명암비가 WCAG AA 에 못 미친다 (bug/minor/frontend)
- SYM-56 선언되지 않은 --danger 토큰을 폴백으로 쓰는 자리 3곳을 토큰 체계에 넣는다 (enhancement/minor/frontend)

### 변경

- 다크 `--accent` `--text-danger` `--color-fail`, 라이트 `--text-danger` `--color-na` 를 4.5:1 이상으로. 색상과 채도는 두고 명도만 조정
- 토큰 명암비 검사 추가. 글자색 토큰이 검사 목록 밖에 있으면 실패하는 인벤토리 단언도 함께 넣었다
...
## v1.6.3.0 (2026-09-18) - [feat] 수행 화면의 Platform 열과 필드 설정 적용

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.2.0 | **1.6.3.0** | fix +1 |
| Frontend | 1.6.1.0 | **1.6.2.0** | fix +1 |
| Backend | 1.6.1.0 | **1.6.2.0** | fix +1 |

### 이슈

- SYM-109 테스트 수행 화면과 런 엑셀에 Platform(test_type) 열 추가 (enhancement/minor/frontend)
- SYM-110 Ctrl+D 가 읽기 전용 TC 열에서 채웠다고 알린다 (bug/trivial/frontend)
- SYM-111 수행 그리드가 프로젝트의 필드 표시 설정을 따르지 않는다 (enhancement/minor/frontend)

### 변경

- 수행 화면과 두 엑셀에 Platform 열 추가. 읽기 전용
- 수행 그리드가 프로젝트 필드 설정을 따른다. TC 에서 오는 아홉 열만 대상이고 수행 전용 열은 늘 보인다
- Ctrl+D 를 채워도 되는 열(결과, 실제 결과, 이슈 링크, 비고)로 한정

### 영향

- 필드 설정에서 숨긴 TC 열은 수행 화면에서도 빠진다. 내보내기 파일에는 그대로 실린다
- Ctrl+D 가 소요(초)에서 동작하지 않는다. 타이머 측정값을 덮어쓰던 경로였다

---

## v1.6.2.0 (2026-09-18) - [feat] 테스트 수행 화면에 사전조건 열

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.6.1.0 | **1.6.2.0** | fix +1 |
| Frontend | 1.6.0.1 | **1.6.1.0** | fix +1 |
| Backend | 1.6.0.1 | **1.6.1.0** | fix +1 |

### 이슈

- SYM-108 테스트 수행 화면에 사전조건 열 추가 (enhancement/minor/frontend)

### 변경

- 수행 그리드에 사전조건 열 추가. 읽기 전용이고, 다른 TC 를 참조하는 사전조건은 `PreconditionCell` 툴팁으로 펼쳐 보인다
- 참조 색인은 런에 담긴 TC 로만 만든다. 색인에 없으면 안내 문구가 뜬다
- 런 엑셀과 리포트 엑셀에 같은 열 추가. 두 파일의 TC 열 집합이 갈라지지 않게 계약 테스트로 묶었다
- 리포트 엑셀의 결과 색칠이 열 번호를 박아 쓰던 것을 헤더 이름 기준으로 바꿈
- 런 엑셀 열 폭 배열이 헤더보다 하나 적어 폭이 밀리던 것을 바로잡음

### 영향

- 수행자가 TC 관리 화면을 열지 않고 사전조건을 읽는다
- 내보낸 파일의 열이 하나 늘어난다. 되가져오기는 `Precondition` 매핑이 이미 있어 그대로 들어간다
...
## v1.6.0.0 (2026-09-17) - [feat] 토큰 폐기 수단, 접근성 검사, E2E 고정 대기 제거

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 |
|---|---|---|
| System | 1.5.7.0 | **1.6.0.0** |
| Frontend | 1.5.5.0 | **1.6.0.0** |
| Backend | 1.5.6.0 | **1.6.0.0** |
| Database | 0.9.1.0 | **0.9.2.0** |

### 이슈

- SYM-100 로그인 유지 토큰이 10년이고 폐기할 수단이 없다 (bug/critical/security)
- SYM-101 접근성 속성이 없어 스크린리더로 쓸 수 없다 (enhancement/major/frontend)
- SYM-102 E2E 가 고정 대기에 의존해 불안정하다 (enhancement/minor/etc)
- SYM-103 CI 의 타입 검사가 아무 파일도 보지 않는다 (bug/major/etc)

### 변경

- `users.token_version` 추가. 토큰의 `ver` 와 요청마다 대조한다
- 비밀번호 변경·관리자 초기화·계정 복구·로그아웃에서 버전을 올려 옛 토큰을 끊는다
- 로그인 유지 3650일 -> 30일
- 스키마 표지에 `token_version` 추가
- 링크 색과 안내 문구 대비 교정(2.96/2.76 -> 6.70/4.83)
- 폼 라벨을 입력과 연결, 모달에 `dialog` 역할, 아이콘 버튼에 `aria-label`
- 검색 드롭다운을 Escape 로 닫는다
- axe-core 검사 15건. CI e2e 에 포함
- CI 타입 검사를 `tsc -b` 로. `tsc --noEmit` 은 검사 대상이 0 개였다
- 회귀 E2E 6개 스펙의 `waitForTimeout` 82건 제거
- 단언이 없거나 조건부로 건너뛰던 테스트 12건에 검증 추가
- 권한 검사 2건을 소스 문자열 검사에서 HTTP 호출로 교체

### 영향

- **모든 사용자가 한 번 다시 로그인해야 한다.** 기존 토큰에 `ver` 가 없어 거부된다
- 로그아웃하면 그 계정의 다른 기기 세션도 끊긴다
- 로그인 유지를 켜도 30일 뒤 만료된다
- `alembic_version` 이 없는 구형 DB 는 기동을 멈춘다

### 조치

- `alembic upgrade head` (`c2f8a90b3d14`)
...
## v1.5.6.0 (2026-09-14) - [fix] 응답 계약과 로케일, 남은 검증 공백

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.5.5.1 | **1.5.6.0** | fix +1 |
| Frontend | 1.5.3.1 | **1.5.4.0** | fix +1 |
| Backend | 1.5.4.0 | **1.5.5.0** | fix +1 |

### 이슈

- SYM-86 리포트 비율의 분모가 서로 달라 합이 100 이 되지 않는다 (bug/minor/backend)
- SYM-87 쓰이지 않는 대시보드 응답 스키마가 붙는 순간 화면을 비운다 (bug/minor/backend)
- SYM-88 최초 관리자 승격에 동시 가입 경쟁이 있다 (bug/minor/security)
- SYM-89 TC ID 에 길이 검증이 없어 컬럼 제한을 넘는 값이 저장된다 (bug/major/db)
- SYM-90 TC 만 남은 이름으로 시트를 바꾸면 이유를 알 수 없는 오류가 난다 (bug/minor/backend)
- SYM-91 시트 이동이 보내지 않은 필드를 바꾼다 (bug/minor/backend)
- SYM-92 영어 화면에 한글 문구가 그대로 나온다 (bug/minor/frontend)
- SYM-93 대시보드 날짜 프리셋이 UTC 기준이라 경계일이 밀린다 (bug/minor/frontend)
- SYM-94 리포트 응답 타입이 실제와 달라 컴파일러가 보호하지 못한다 (bug/minor/frontend)

### 변경

- 리포트 요약·PDF·엑셀에 `executed`(pass+fail+block) 표기. `pass_rate` 의 분모를 드러낸다
- 쓰이지 않던 대시보드 응답 스키마 5종 제거
- 가입 직후 관리자 판정을 다시 해 경쟁에서 진 쪽을 되돌림(SQLite 전제, 주석에 명시)
- TC ID 1~50자 검증(생성·수정). 응답 스키마에는 걸지 않는다
- 시트 이름 변경의 번호 충돌을 이유 있는 400 으로. 미리 막으면 부딪히지 않는 경우까지 막는다
- 시트 이동은 보내지 않은 필드를 건드리지 않음
- 내보내기 모달과 사전조건 안내를 i18n 으로, 날짜 프리셋을 로컬 기준으로
- 리포트 전용 응답 타입 정의

### 영향

- 리포트 응답·PDF·엑셀에 Executed 칸이 늘어남
- 50자를 넘는 TC ID 는 422. 실 DB 최장이 14자라 기존 값은 영향 없음
- 영어 화면의 내보내기 모달과 사전조건 안내가 영어로 나옴. 엑셀 내보내기의 백엔드 문구는 한글 유지
...
## v1.5.5.0 (2026-09-14) - [fix] 저장 유실과 화면 표시, 저장형 XSS

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.5.4.0 | **1.5.5.0** | fix +1 |
| Frontend | 1.5.2.0 | **1.5.3.0** | fix +1 |

### 이슈

- SYM-73 수행 그리드에서 0.3초 안에 다른 행을 편집하면 앞 행 저장이 취소된다 (bug/major/frontend)
- SYM-74 타이머가 잰 소요 시간이 저장되지 않는다 (bug/major/frontend)
- SYM-75 프로젝트 설명을 지우면 저장되지 않는데 성공 메시지가 뜬다 (bug/major/frontend)
- SYM-76 아이디에 든 HTML 이 관리자 화면에서 실행된다 (bug/major/security)
- SYM-77 영어로 쓰면 새 행의 우선순위가 번역문으로 저장된다 (bug/major/frontend)
- SYM-78 찾기 바꾸기가 필터로 숨은 행까지 바꾸고 수행 그리드 메모의 줄바꿈이 사라진다 (bug/major/frontend)
- SYM-79 관리자 매뉴얼 안내 박스가 다크모드에서 읽히지 않는다 (bug/major/frontend)
- SYM-80 커스텀 필드 컬럼의 Ctrl+D 와 되돌리기가 값을 바꾸지 않는데 완료 메시지가 뜬다 (bug/minor/frontend)
- SYM-81 비교 화면에서 늦게 온 응답이 최신 결과를 덮는다 (bug/minor/frontend)
- SYM-82 대시보드 담당자별 표가 늘 비어 있고 일괄 삭제는 전부 실패해도 성공 메시지를 띄운다 (bug/minor/frontend)
- SYM-83 i18n 키 누락과 미선언 CSS 토큰, 필터 아이콘 대비 (bug/minor/frontend)
- SYM-84 버전 표기가 실제 릴리즈와 어긋나고 언어를 바꿔도 수행 그리드 헤더가 그대로다 (bug/trivial/etc)

### 변경

- 수행 결과 저장 타이머를 행별로 분리하고 언마운트 시 정리. 타이머가 잰 시간을 저장 경로에 연결
- 프로젝트 설명을 빈 문자열로 전송해 지운 값이 반영되게 함
- 사용자명을 `Trans` 로 렌더해 저장형 XSS 제거. 새 행 우선순위는 DB 값 고정
- 찾기 바꾸기를 보이는 행으로 한정, 수행 그리드 메모에 큰 편집기 적용, 커스텀 필드 값 접근을 `rowFields` 로 통일
- 비교 화면에 세대 번호 추가, 담당자별 표 제거, 일괄 삭제 실패를 실패로 표시
- 정적 게이트 추가: i18n 키 존재, 폴백 뒤 미선언 CSS 토큰, 버전 표기 정합

### 영향

- 수행 화면에서 연속 입력이 유실되지 않음. 소요 시간이 엑셀과 리포트에 기록되기 시작
- 다크모드 관리자 매뉴얼 대비 1.14:1 -> 정상, 필터 아이콘 1.03:1 -> 약 7:1
- 대시보드에서 담당자별 표가 사라짐(v1.2.0 이후 늘 비어 있던 표)

---

## v1.5.4.0 (2026-09-14) - [fix] 집계 기준 정립과 입력 검증, 스키마 제약

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.5.3.0 | **1.5.4.0** | fix +1 |
| Backend | 1.5.3.0 | **1.5.4.0** | fix +1 |
| Database | 0.9.0.0 | **0.9.1.0** | fix +1 |

### 이슈

- SYM-63 대시보드 전체 모드가 삭제된 TC 의 결과를 분자에 센다 (bug/major/db)
- SYM-64 대시보드가 다른 프로젝트 수행의 집계를 돌려준다 (bug/major/security)
- SYM-65 지운 TC 의 수행 기록이 7일 뒤 사라져 리포트 총계가 달라진다 (bug/major/db)
- SYM-66 커스텀 필드 수정 API 에 타입 검증이 없다 (bug/major/db)
- SYM-67 시트를 옮긴 TC 가 수행에 남아 결과를 저장할 수 없다 (bug/major/backend)
- SYM-68 TC 목록 엑셀이 시트 차례를 무시하고 번호로만 정렬한다 (bug/major/backend)
- SYM-69 임포트가 폴더 시트에 TC 를 넣고 벌크 삭제가 500 을 낸다 (bug/minor/backend)
- SYM-70 멤버와 시트 이름 중복을 DB 가 막지 않는다 (bug/major/db)
- SYM-71 업로드 크기 제한이 버퍼를 다 채운 뒤에야 걸리고 삭제된 TC 를 수정할 수 있다 (bug/minor/security)
- SYM-72 Alembic 이전 DB 를 검증 없이 최신으로 표시한다 (bug/major/db)

### 변경

- 집계 기준 분리: 대시보드는 활성 TC 만, 리포트와 수행 상세는 수행 시점 기록 유지. 수행에 편입된 TC 는 기한이 지나도 보존
- 검증 추가: 커스텀 필드 타입, 벌크 삭제 ids, 삭제 TC 수정, 임포트 폴더 시트, 업로드 청크 중단, Alembic 이전 DB 스키마
- 유니크 인덱스 2종(마이그레이션 `b7c31d9e42f8`), TC 목록 엑셀을 `leaf_sheet_order` 로 정렬

### 영향

- 대시보드 합격률 변동. 삭제 TC 가 있는 프로젝트에서 실측 40.0% -> 30.0%
- 리포트와 대시보드 총계가 다를 수 있음. 기준이 다르며 각각은 일관됨
- 마이그레이션이 중복 멤버·시트를 정리하며 되돌릴 수 없음. 실 DB 는 중복 0건이라 삭제 없음

---

## v1.5.3.0 (2026-09-11) - [fix] TC 0건 프로젝트의 시트 분리 내보내기 실패

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.5.2.0 | **1.5.3.0** | fix +1 |
| Backend | 1.5.2.0 | **1.5.3.0** | fix +1 |

### 이슈

- SYM-60 TC 가 없는 프로젝트에서 시트 분리 내보내기가 500 으로 실패한다 (bug/major/backend)
- SYM-61 시트명이 31자를 넘고 서로 겹치면 엑셀 탭 이름이 32자가 된다 (bug/minor/backend)

### 변경

- 시트 분리 모드에서 만들 탭이 없으면 통합 모드와 같은 Test Cases 시트 한 장 배치. 기본 시트를 지운 뒤 탭이 0개면 openpyxl 이 저장 단계에서 IndexError 를 내 500 으로 이어짐
- 시트명 정제를 _safe_sheet_title 한 자리로 통합. 정제, 31자 자르기, 중복 회피가 흩어져 있어 앞 31자가 같은 이름끼리 충돌 시 탭 이름 32자(실측 [5, 3, 4, 31, 32])
- 금지 문자만 남아 이름이 비면 기본 으로 대체. 종전에는 openpyxl 이 Sheet 로 바꿔 원본 식별 불가

### 영향

- TC 를 아직 넣지 않은 프로젝트에서도 두 모드 모두 파일 다운로드 가능. 전체 TC 가 소프트 삭제된 프로젝트도 동일
- 기존 프로젝트의 탭 이름 변동 없음. 토스증권 웹 QA 9개 탭 이름이 수정 전후 동일
- 런 엑셀과 리포트 엑셀은 시트를 항상 만들어 해당 없음

---

## v1.5.2.0 (2026-09-11) - [fix] 리포트의 미수행 미표시와 소문자 결과값 집계 누락

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.5.1.1 | **1.5.2.0** | fix +1 |
| Frontend | 1.5.1.1 | **1.5.2.0** | fix +1 |
| Backend | 1.5.1.0 | **1.5.2.0** | fix +1 |

### 이슈

- SYM-57 리포트 화면이 미수행과 N/A 를 표시하지 않는다 (bug/major/frontend)
- SYM-58 결과값이 소문자로 저장된 행 107건이 집계에서 통째로 빠진다 (bug/critical/db)
- SYM-59 푸터 버전 문구가 실제 릴리즈 버전과 다르다 (bug/minor/frontend)

### 변경

- 리포트 요약 카드에 PASS 건수와 N/A, 미수행 추가. 종전 네 장(전체 TC, PASS Rate, FAIL, BLOCK) 구성에서 run 27 의 미수행 15건과 run 30 의 134건이 화면에 없음
- 카테고리별 요약 표에 미수행 컬럼 추가. 행마다 다섯 칸의 합이 Total 과 일치. PDF 의 Category Breakdown 표에도 NA, NS 컬럼 추가
- PASS Rate 카드에 분모 표기(PASS+FAIL+BLOCK). 397건 중 23건만 수행한 런이 100% 로 읽히는 것을 방지
- 소문자 결과값 107건 대문자 교정. SQLite 의 `=` 는 대소문자를 구분하므로 run 25 와 run 26 은 Total 만 맞고 나머지 다섯 칸이 0
- 미수행 색을 텍스트용과 악센트용으로 분리. 대시보드 값을 그대로 쓰면 흰 배경 대비 1.47:1 로 표의 숫자 판독 불가

### 영향

- 리포트 탭과 PDF 리포트에 미수행과 N/A 노출. 기존 런의 수치 변동 없음, 감춰져 있던 칸만 드러남
- run 25 는 PASS 23 / BLOCK 2, run 26 은 PASS 80 / FAIL 1 / BLOCK 1 로 집계(교정 전 다섯 칸 모두 0)
- 푸터 표기 v1.5.2.0

### 조치

- 없음. 데이터 교정 적용 완료, 스키마 변경 없음

---

## v1.5.1.1 (2026-09-10) - [fix] 선언되지 않은 CSS 토큰으로 버튼 글자가 사라지던 문제

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.5.1.0 | **1.5.1.1** | patch +1 |
| Frontend | 1.5.1.0 | **1.5.1.1** | patch +1 |

### 이슈

- SYM-54 대시보드 날짜 필터의 선택된 버튼 글자가 보이지 않는다 (bug/major/frontend)

### 변경

- 선언되지 않은 토큰을 참조하던 9곳이 선언된 토큰을 쓴다. 폴백 없는 미선언 `var()` 는 그 속성을 통째로 무효로 만들어, 선택 버튼의 배경이 transparent 가 되고 흰 글자만 남았다
- 미선언 토큰 참조를 잡는 테스트를 붙였다. tsc 도 eslint 도 문자열 안을 보지 않아 이 결함은 게이트를 그대로 통과한다. 프론트 451건 통과

### 영향

- 대시보드 날짜 필터, TC 이력 표와 내보내기 모달, 사용자 매뉴얼 코드블록, 프로젝트 설정 입력칸이 라이트와 다크 양쪽에서 테마 값을 따른다

---

## v1.5.1.0 (2026-09-09) - [fix] 임포트가 파일 번호에 걸려 실패하던 문제

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.5.0.0 | **1.5.1.0** | fix +1 |
| Frontend | 1.5.0.0 | **1.5.1.0** | fix +1 |
| Backend | 1.5.0.0 | **1.5.1.0** | fix +1 |

### 이슈

- SYM-52 파일 안에 같은 번호가 있으면 CSV 와 마크다운 임포트가 통째로 실패한다 (bug/major/backend)
- SYM-53 개발 DB 를 지키는 테스트 가드가 실제 요청 대상을 보지 않는다 (bug/major/etc)

### 변경

- CSV 와 마크다운도 파일의 No 를 차례로만 쓴다. 앞 릴리즈에서 엑셀만 고쳐, 나머지 둘은 파일 안에 같은 번호가 있으면 409 로 죽었다
- 번호를 비켜 두는 자리를 그 시트의 최대 절댓값보다 크게 잡는다. 고정 상수로 두면 데이터에 그 값이 들어왔을 때 부딪친다
- 개발 DB 위험 판정이 테스트가 실제로 요청을 보내는 곳을 본다. 판정과 요청 대상이 갈라지면 잡히는 테스트를 붙였고, 그 테스트가 가드 없는 파일 2개를 찾아냈다
- 정렬이 걸린 상태의 드래그를 저장 직전에 거절한다. 행 수가 같아 개수 비교로는 걸리지 않는다
- 테스트 12건 추가. 백엔드 345건, 프론트 449건 통과

### 영향

- 파일 안에 번호가 겹치거나 비어 있어도 임포트가 받아들인다. 저장되는 번호는 파일과 무관하게 1..N 이다
- 컬럼 정렬 상태에서는 드래그 정렬이 거절된다. 정렬을 풀고 해야 한다

---

## v1.5.0.0 (2026-09-09) - [refactor] TC 번호 관리를 서버와 DB 제약으로

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.4.1.0 | **1.5.0.0** | feature +1 |
| Frontend | 1.4.1.0 | **1.5.0.0** | feature +1 |
| Backend | 1.4.1.0 | **1.5.0.0** | feature +1 |
| Database | 0.8.0.0 | **0.9.0.0** | feature +1 |

### 이슈

- SYM-45 라운드가 비어 있는 수행이 하나라도 있으면 그 프로젝트의 수행 목록이 500 이다 (bug/major/backend)
- SYM-46 기존 TC 의 no 가 시트 안 순번 규약을 만족하지 않는다 (bug/major/db)
- SYM-47 동시에 복제하면 같은 시트에 같은 no 가 들어갈 수 있다 (bug/minor/backend)
- SYM-48 수행 엑셀과 리포트 엑셀에 수식 인젝션 새니타이즈가 없다 (bug/minor/security)
- SYM-49 테스트 40건이 CI 에서 건너뛰어지고 있었다 (bug/major/etc)
- SYM-50 TC 번호를 넣는 경로가 시트 안 순번 규약을 지키지 않는다 (bug/major/db)

### 변경

- `no` 는 살아 있는 TC 의 시트 안 순번이다. 부분 유니크 인덱스로 DB 가 강제하고, 기존 데이터를 시트마다 1..N 으로 다시 매겼다
- 번호를 넣는 네 경로를 서버가 통제한다. 신규 생성과 수정은 보낸 값을 쓰지 않고, 임포트는 파일의 No 를 차례로만 쓰고, 정렬은 한 시트 전체를 1..N 으로 보낼 때만 받는다
- 라운드가 비어 있어도 수행 목록이 열린다. 컬럼을 NOT NULL 로 바꾸고 2건을 채웠다
- 수행 엑셀과 리포트 엑셀도 수식으로 읽힐 값을 막는다
- 테스트 건너뛰기 판정을 포트가 아니라 격리 여부로 바꿨다. 기본 조건에서 도는 백엔드 테스트가 262건에서 332건이 됐다(45건 추가). 프론트 445건 통과

### 영향

- TC 관리 화면과 TC 목록 엑셀의 번호가 달라진다. 실 DB 23개 시트 중 13개가 규약을 어겼다. TC 총계와 순서는 그대로다
- No 는 읽기 전용이다. 순서는 드래그로 바꾸고, 드래그는 시트 탭에서 필터 없이 볼 때만 켜진다

### 조치

- 마이그레이션은 서버 기동 시 자동 적용된다. 정규화 전 번호는 백업 테이블에 남는다

---

## v1.4.1.0 (2026-09-09) - [fix] 수행 화면과 엑셀의 No 를 목록 순번으로 통일

### 컴포넌트 버전

| 컴포넌트 | 이전 | 이후 | 변경 |
|---|---|---|---|
| System | 1.4.0.1 | **1.4.1.0** | fix +1 |
| Frontend | 1.4.0.0 | **1.4.1.0** | fix +1 |
| Backend | 1.4.0.0 | **1.4.1.0** | fix +1 |

### 이슈

- SYM-41 시트를 골라 만든 테스트 수행의 No 가 1 부터 시작하지 않는다 (bug/major/frontend)
- SYM-42 TC 복제가 프로젝트 전체 max(no)+1 을 줘서 시트 안 번호에 구멍이 난다 (bug/major/backend)
- SYM-43 수행 이름에 한글이 있으면 엑셀 내보내기가 500 이다 (bug/major/backend)
- SYM-44 같은 수행인데 수행 엑셀과 리포트 엑셀의 No 와 행 순서가 다르다 (bug/minor/backend)

### 변경

- 수행 화면의 No 를 시트 탭과 전체 탭 모두 1 부터 매긴다. 시트 순서와 기본 탭도 트리 루트가 아니라 잎 시트에서 고른다
- TC 복제가 시트 기준 max(no)+1 을 준다. 지운 TC 의 번호는 쓰지 않는다
- 두 엑셀이 같은 차례와 번호를 내고, 수행 엑셀의 Content-Disposition 을 RFC 5987 로 바꿨다
- 테스트 15건 추가. 백엔드 287건, 프론트 440건 통과

### 영향

- 수행 화면과 두 엑셀의 No 가 달라진다. 저장된 no 는 그대로다 (TC 관리 화면의 구멍은 SYM-46)
- 시트를 폴더로 묶은 프로젝트는 기본 탭이 전체에서 첫 시트로 바뀐다
