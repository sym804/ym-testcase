...
★파싱 규칙은 두 곳이 같지만 **반환 문구는 프론트만 i18n 으로 옮겼다**(frontend/src/utils/precondition.ts). 여기서 내는 한글 문구는 엑셀 내보내기를
타고 셀에 그대로 들어가므로, 영어 사용자가 받는 파일에는 한글이 섞인다.
서버 사이드 i18n 이 없어 남겨 둔다. 고치려면 요청의 Accept-Language 를 받거나
언어 중립 마커를 내고 소비자가 푸는 방식이 필요하다.
"""
